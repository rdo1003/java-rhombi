...
*************************************************************************/
int countShapeHorizontal = 0;    //countX
int countShapeVertical   = 0;   //countY
...
void setup() {
  ...
  countShapeHorizontal = width / int(50.0);    //Number of shapes in the x direction to fit screen
  countShapeVertical = height / int(20.0);     //Number of shapes in the y direction to fit screen
  //For testing:
...
void draw() {
  ...
  if(colorUp) {
    ...
    colorCount -= colorSpeed;                //cycles through red and blue gradients
  }
  if(colorCount > 200) {
    colorUp = false;
  ...
  for(int y = -30; y < (countShapeVertical * 20) - 30; y += 20) {
    for(int x = -75; x < (countShapeHorizontal * 50) - 75; x += 50) {
      ...
      if(backslash) {
        ...
        fill(red, green, blue);
        beginShape();
          vertex((x + 10) * m, (y + 10) * m); //vertex:every side of rhombi
